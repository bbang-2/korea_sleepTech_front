...
//! 프로그램 설정 순서 (터미널에서 대화형으로 진행)
// project name: 프로젝트 폴더명
// package name: pakage.json의 "name"의 속성값으로 지정 (소문자 작성 !!)
...
//    - App.tsx: 주요 컴포넌트
//    - main.tsx: 앱 진입점
// 4. index.html: HTNL 템플릿
//    : React는 S(single)P(page)A(application)
// 5. package.json: 프로젝트 정보 & 의존성
// 6. tsconfig.json: Typescript 속성
// 7. vite.config: vite 설정
...
// 3. npm run preview: 빌드 된 앱 미리보기 (build 후 사용 가능)
...
/*
  ...
  "devDependencies": { //! 애플리케이션 동작과는 무관하지만 개발 시 필요한 의존성 라이브러리
    ...
    "@vitejs/plugin-react": "^4.3.4", //? React 플러그인 (JSX 지원)
    "eslint": "^9.21.0",
...
// - 개발중에도 해당 파일을 기쥰으로 동작 (vite가 해당 파일을 분석하여 앱실행)