...
//! === 장바구니 구현 === //
// : 배열 렌더링 (추가, 조회, 수정, 삭제)
// - 해당 과정에사 배열 내부의 각 요소를 구분짓는 id값을 포함한 객체
...
interface IItemProps {
  item: IItem;
  // 장바구니 수정: 수정 할 고유 아이템의 id값, 수정할 수량 데이터
...
function Practice() {
  ...
  const handleCreateItem = (name: string, amount: number) => {
    ...
    nextId.current++; // item(상태 변수)의 변경 여부와 상관 없이 값이 유지
  };
  ...
  const hamdleRemoveItem = (id: number) => {
    ...
    >
      ...
      */}
      ...
      {items.map((item) => (
        ...
        // : 생성되는 컴포넌트 또는 요소에 각각의 구분할 수 있는 key값을 전달
        // >> ket 값은 각 요소를 분리할 수 있는 고유값 지정 권장
        // <Item item={item} key={index} />
        <Item
          item={item}
          key={item.id}
          onRemove={hamdleRemoveItem}
