...
/*
  ! === useState ===
  : Recat에서 제공하는 Hooks 중 하나
  '함수형 컴포넌트' 내에서 상태를 관리하는 기능
  >> 컴포넌트 단위에서의 상태 관리

  : useState 훅 호출 시
  >> 변회되는 상태값과 해당 상태값을 업데이트하는 함수를 제공
  >> 상태 업데이트 함수는 '비동기적' 처리가 기본으로 진행
  >> 상태 변경 시 '컴포넌트 재랜더링'을 위반
*/
...
function UseState01() {
  ...
  //? 상태 설정 함수를 그대로 사용
  // : 이전의 상태를 직접 참조
  // - 주로 현재(이전)의 값과는 관련이 없는 변화가 이루어질 경우 사용
  const handleUpClick = () => {
    // 해당 이벤트 핸들러 실행 시 컴포넌트 전체의 상태에 대한 증가만 이루어짐
    setCount(count + 1) // 0 + 1
    // setCount(count + 1) 0 + 1 - 바로 직전의 상태 변화를 감지하지x
  ...
  return (
    <div>
      ...
      <button onClick={handleUpClick}>카운트 2씩 증가</button>
      <button onClick={handleDownClick}>카운트 2씩 감소</button>
