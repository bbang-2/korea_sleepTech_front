...
function UseState02() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    // input창에 change 변화가 일어나면 처리(handle)할 로직
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // 매개변수로 전달 받는 e(이벤트 객체)의 target >> 요소
    // : target 요소 내의 속성에 접근 가능
    
    // e.target
    // : name과 value값을 추출
    // - name값 지정 시 상태변수와 이름을 잋리
    // - value값은 사용자로부터 입력 받는 값
  ...
  const handdleResetLogin = () => {
    setLogin(loginInitialValue);
  }

  const handleLoginSubmit = (e: React.FormEvent) => {
    e.preventDefault(); // 폼 기본동작 제출 방지
  ...
  return (
    <div>
      ...
      <button onClick={handdleResetLogin}>초기화 버튼</button>
