...
const initialFormData: IFromData = {
  id: "",
  password: "",
  email: "",
};
...
function UseState04() {
  //! 폼 데이터 상태 관리
  const [formData, setFormData] = useState<IFromData>(initialFormData);

  const { id, password, email } = formData;

  //! 폼 입력 오류 메시지 상태 관리
  const [errors, setErrors] = useState<IFromData>(initialFormData);
  ...
  const onSignUpSubmit = (e: React.FormEvent) => {
    ...
    if (!id) { // id 입력 필드가 비워진 걍우 (빈 문자열 - false)
