...
// - 부모로부터 사용자에 대한 데이터를 전달(props)받아 UI로 변한
// - props는 객체의 형태!
...
// props는 변수, 객체 상관 없이 무조건 {} 객체 내에 작성
// - 객체 데이터 틀로 받아오려면 본인의 객체 틀 외에 또 다른 객체 {}틀로 묶여야함
// type UserCarProps = {{name, age, email}: User};
...
export const Wrapper = ({ children }: ChildrenType) => {
  return (
    <div style={{ border: '2px solid black', padding: '16px', backgroundColor: 'lightgreen' }}>
      {/* props로 전달받은 UI를 HTML내에서 출력 시 JS의 문법 출력처럼 사용 */}
      {children}
    </div>
  )
}
  ...
  return (
    ...
    <Wrapper>
      {/* 
        ...
        : main.tsx에서 명시
        - 렌더링 시 컴포넌트를 일부러 콘솔창에 두 번 생행 (부작용 확인)
      */}
      <UserCard user={{ name: '이승아', age: 30, email: 'qwe23'}} />
