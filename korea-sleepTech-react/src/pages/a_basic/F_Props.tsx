...
/*
  ...
  ! Props 전달
  : 부모 컴포넌트로부터 자식 컴포넌트로 데이터를 전달 할 때 사용
...
function MultiProps ({ name = '김명진', colorProps }: MultiPropsType) {
  // 여러개의 props가 전달되더라도 하나의 객체로 받음
  // : 위 처럼 객체나 배열 내부의 요소들을 한번에 각각의 요소 변수에 할당하는
  //!    , 구조분해할당(바구조화 할당) 가능

  //! 기본 속성값 지정
  // : 컴포넌트 호출 시 props를 전달하지 않을 경우 지정될 기본 값
  // - 매겨 변수에서 지정
  // - 속성명 = '기본값'
  return(
  ...
  return (
    <div>
      {/* 
        ...
        !! props의 경우 자식 컴포넌트에 객체형식으로 데이터를 전달
        >> 자식 컴포넌트에 매개변수 지정 시 타입을 객체 별칭으로 지정
      */}
      <Wrapper>
      ...
      {/* 여러개의 props 전달 */}
      <MultiProps name='윤영서'colorProps='lightblue'/>
