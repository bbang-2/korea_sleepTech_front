...
/*
  ! JSX(TSX)
  : JS(TS) 파일 내에서 HTML과 유사한 마크업을 장성 할 수 있도록 해주는 JS구문 확장 문법
  ...
  - JSX내에서 HTML 코드와 JS코드를 분리변환 할려면 XML문법을 준수
  ...
  2) 태그 닫기
  : 빈 태그(void 태그, 단일 태그) 사용 시 닫히는 꺽쇠괄호에 / 가 첨수
  - input, br, hr, img
...
//! JSX내에서 자바스크립트 값 사용
// : JSX내에서 중괄호 사용 방법 2가지

// 1. JSX태그 내에서 직접적인 데이터 사용
// 2. '속성=' 기호 바로 다음
  ...
  return (
    <div>
      ...
      */}
      ...
      {/* 
        요소에 이벤트 전달 시, 코드를 해석하는 과정에서 실행되지 않ㄷ록
        >> 콜백함수의 형태로 전달 (익명함수의 형태)
        >> 해당 이벤트가 발생 해야만! 내부 콜백 함수가 실행!
      */}
      <button onClick={() => handleClick('클릭!')}>클릭해주세요.</button>

      <div style={divStyle} id='divElement' className='divClassElemnet'>
