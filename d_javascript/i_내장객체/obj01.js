...
console.log(num.toFixed()); // 123: 자릿수 지정x -> 소수점 전체 반올림

// 2) isNaN(), isFinite()
// : 각각 NaN값, Infinity값인지 확인
// - Number 객체에서 호출
let notNum = Number('숫자로 변할 수 없는 값'); // NaN

console.log(notNum);
console.log(notNum === NaN); // false: NaN 값은 비교 연산자 사용 x
...
mantData = mantData.trim();

// 데이터 내부를 줄 바꿈으로 나누고 각 요소를 배열로 반환
// : 줄바꿈 기호 '\n'
mantData = mantData.split('\n');

console.log(mantData);

mantData = mantData.map(line => line.trim().split(','));
