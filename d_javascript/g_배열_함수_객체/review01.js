...
//# Tod 리스트 만들기 (할 일 목록 관리 시스템) //

//! Todo 객체 구성
// - id: 각 할 일의 고유 식별자 (주로 숫자나 문자열로 표현)
// - content: 할 일의 고유 내용을 문자열로 저정
// - completed: 할 일의 완료 상태를 나타내는 boolean 테이터

/* 
let todo = {
  id: 1,
  content: '정보처리 기능사 필기 원서 접수',
  completed: false
}
*/

//! === 프로젝트 개요 === //
// Todo 객체 데이터에 대한 CURD 기능
...
function toggleTodo (id) {
  ...
  todos = todos.map(todo => {
    ...
    if (todo.id === id) {
      ...
      // let arr1 = [1, 2, 3];
      // let arr2 = [...arr1]; >> arr의 요소만 가져와서 새로운 배열 생성 (메모리 주소가 다름)

      // 객체명.속성명 = 속성값; >> 객체 속성값 재할당 || 객체의 속성 생성
...
// 3) 할 일 삭제 함수
// 삭제하고자 하는 id를 가진 할일을 todos 배열에서 제거
// > 배열 내부 요소 개수의 변화o (filter)
function deleteTodo (id) {
  // 1, 2, 3, 4, 5 중 4를 제거
  // 4와 일치하지 않는 1, 2, 3, 5 민을 새로운 배열로 저장
  // >> 4가 삭제된 효과
