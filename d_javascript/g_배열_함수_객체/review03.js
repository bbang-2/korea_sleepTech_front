...
class Book {
  ...
  constructor(id, title, author) {
    // 생성자 함수에서 thisㅋ워드로 속성 정의되는 값은 필드로 정의!
    this.id = id;
    this.title = title;
    this.author = author;
...
class Library {
  ...
  displayBooks() {
    ...
    this.books.forEach(book => {
      console.log(`${book.id}: ${book.title} by ${book.author} - ${book.isAvailable ? '대여가능' : '대여 중'}`);
    })
  }
  ...
  updateBook(id, newTitle, newAuthor) {
    // find 메서드
    // : 배열에사 제공 된 콟백함수를 만족하는 첫번째 요소를 반환
    // ; 만족하는 값ㅇ 없으면 undifined를 반환
    const book = this.books.find(book => book.id === id);
  ...
  removeBook(id) {
    ...
    if(index !== -1) {
      ...
      // : 1번 인덱스 요소부터 개 삭제
      // > [2, 3]
      const removedBook = this.books.splice(index, 1)[0]; // [삭제 할 요소]
      console.log(`${removedBook.title} (id: ${removedBook.id}) 책이 도서관에서 삭제 되었습니다.`);
    } else {
