...
// button1.addEventListener('click', message); // 'button' 요소에 클릭 이벤트 발생 시 message 기능 적용
// button2.addEventListener('click', message); // 'button' 요소에 클릭 이벤트 발생 시 message 기능 적용
...
/* 
  ! 3. 함수 구현 방법
    ...
    ? 함수 명명 규칙: lowerCamelCase 시용, 동사 사용 권장!
...
// >> '호이스팅' 작용 x: 선언(정의) 이전에 호출 불가

/* 
  ? cf) 함수를 할당하는 변수는 let, const 모두 가능
  const 변수명 = function(매기변수) {
    - 힘수 기능 작성
    - return 반환값; (선택적)
  }
...
//& 3) 화살표 함수: 기존의 function 키워드를 화살표(=>)로 대체(건결한 작성)
// >> '호이스팅' 적용 x
// +) 항상 익명함수로 작성
//    'this'가 일반함수와 다르게 바인딩 됨

/* 
  let 변수명 - (매개변수 - prameter) => {
    - 함수 기능 작성
    - return 반환값;
  }
*/
...
// console.log(job); - Error(지역변수는 해당 스코프를 벗어나면 사용 불가)
