...
//    >> 실제 데이터x, 메모라 주소를 저장하여 '참조'

//? 요소 길이
console.log(sports.length); // 3

// cf) JS 배열 동적 배열: 임의로 배열의 크기를 수정
//    >> 새로운 공간 타입은 'undifined'
sports.length = 6;
console.log(sports[3]); // undifined
...
// 2) lateIndexOf(요소값): 배열의 끝부터 찾는 요소의 첫번째 인덱스를 반환 (없으면 -1)
console.log(snacks.lastIndexOf('초코송이')); // 3

// 3) includes(): 배열에 해당 요소의 존재 여부 확인
let hashPotato = snacks.includes('포테토칩');
