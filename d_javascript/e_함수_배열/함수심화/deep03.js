...
/* 
  reduce 메서드의 인자! 값: 콜백함수, 초기값(initialValue)
...
// reduce의 축적값: 각 순회마다 축적 되는 값(이전 작업물의 반환 값, 첫번째 호출에서는 '초기값' 지정)
...
console.log(sum); // 10
console.log(sum2); // 110 -> 시작점이 100이라 100 + acc

// == reduce 예시 == //
let cars = ['audi', 'bmw', 'hyundai', 'kai'];

let combinedCar = cars.reduce((acc, car) => {
  return acc + car + ', ';
}, "Cars: ");

console.log(combinedCar) // Cars: audi, bmw, hyundai, kai,
...
// findIndex(): 특정 조건을 만족하는 첫번째 요소의 인덱스 반환
