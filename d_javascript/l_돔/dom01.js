...
//! 1. DOM 정의
// : 웹페이지를 문서 객체로 조작하고 관리 할 수 있는 인터페이스(상호작용, 규약)
...
// : JS내에서 문서 객체 조작 시, HTML의 모든 문서 구조(내용)가 모드 로드된 이후에 이벤트 발생
...
// 2-1) 전체 HTML구조를 객체로 인식하여 속성을 가져옴(중첩 객체)
// ex) document.head / document.title

// 2-2) querySelector() & querySelectorAll() 메서드
// : 선택자를 활용하여 단일 혹은 여러개의 요소를 읽어올 때 사용
// : 태그, #아이디, .클래스, 선택자A 선택자B (자식 요소 선택자)
document.addEventListener('DOMContentLoaded', () => {
  // querySelector()
  // : 요소를 하나만 추출(여러개일 경우 제일 상단의 요소만 가져오기)
