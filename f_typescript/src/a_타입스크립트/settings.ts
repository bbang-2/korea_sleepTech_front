...
- 맥(Mac OS)
  ...
  ? npm 기본 명령어
    1. npm init
      ...
      - 'y옵션 추가' (npm init -y)
        : 질문없이 기본값으로 package.js 파일 생성

    2. npm install
      : pacakge.json 파일에 명시 된 모든  "의존성"을 설치
      - 특정 패키지 설치 방법
        : npm install 패키지명
      - '-D | --save-dev 옵션 추가'
        : 개발 의존성으로 패키지를 추가
  ...
  ! 3. typescript
  : 타입스크립트를 설치 (npm 사용)
  : 타입스크립트 사용을 위해서는 로컬 | 전역 환경에 TS설치가 필수!
  ...
  - 브라우저와 Ndoe.js가 "TS 파일을 직접 실행x" 때문에 tsc로 코드 변환이 필수!

  ? tsc 기본 명령어
  - tsc 파일명.ts
    : 동일한 파일명의 .js 파일이 생성
    : 해당 .js파일은 TS코드에서 타입 정보가 제거된 상태
