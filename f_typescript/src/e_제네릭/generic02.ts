...
// console.log(constraints<boolean>(true)); // 'boolean' 형식이 'ILength' 제약 조건을 만족하지 않습니다.
console.log(constraints<string>('안녕하세요')); // 5

console.log(constraints({
  length: 10, // 필수 속성만 명시 되어있으면 가능 - length 속성을 반드시 포함! (구조적 타이핑)
  value: 3,
  addedOption: 'hi'
}));
...
type Check<T> = T extends string? 'String' : 'Not a String'; // 타입에따라 리터럴 타입을 반환
