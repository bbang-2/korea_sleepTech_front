...
function updeteUser(id: number, changes: Partial<User>) {
  const user = users[id]; // User 타입의 객체: 객체의 속성 값이 number이기 때문에 id값으로 지정
  ...
  // 3) name과 age 둘 다 있는 경우: {naeme, age, name, age} >> 모든 값이 changes 로 수정
  // 4) name과 age 둘 다 없는 경우: 수정 x
  users[id] = {...user, ...changes}
}

updeteUser(1, {name: '윤영서'});
console.log(users[1]); // { name: '윤영서', age: 20 }

updeteUser(2, {age: 30});
console.log(users[2]); // { name: '성재원', age: 30 }

updeteUser(3, {name: '문창배'}); // 해당 id의 사용자는 없습니다.

updeteUser(1, {name: '전창현', age: 25});
