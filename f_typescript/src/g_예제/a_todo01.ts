...
/*
  ...
  ! 요구 사항 정리
    ...
    - 할 일 리스트를 관리 할 수 있는 함수 구현
      : 각 함수는 Todo 항목 배열(todos)를 입력 받고 변경괸 배열을 반환
      1) addTodo
      2) completedTodo
      3) deleteTodo
*/
...
function addTodo(todos: ITodoItem[], task: string): ITodoItem[] {
  const newTodo: ITodoItem = {
    ...
    // - 인자 내에 배열이 포함된 경우 배열 요소 중에 가장 큰 값 반환

    // id: Math.max(0, ...todos.map(todo => todo.id))
    // : 현재 할 일중 가장 큰 값을 반환

    // cf) ...todos.map(todo => todo.id)
    // : 현재의 할 일 목록을 순회하여 id값만 추출

    // 첫번째 인자 0은 리스트가 비워진 경우의 기본 값
    // : 가장 작은 id값은 1
...
function completedTodo(todos: ITodoItem[], id: number): ITodoItem[] {
  const changeTodos: ITodoItem[] = todos.map(todo => 
    ...
    // : 같을 경우 순회되는 요소의 completed 속성만 변셩
    // : 다를 경우 변경없이 반환
...
todos = addTodo(todos, '자바 스크립트 공부');
...
// { id: 2, task: '자바 스크립트 공부', completed: false },
